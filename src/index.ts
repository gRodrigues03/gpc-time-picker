...
export default function onRender(args: FrontendRendererArgs): void {
    ...
    let input = parentElement.querySelector(".cc-input") as HTMLInputElement;
    ...
    if (!input) {
        ...
        input.addEventListener("blur", (e) => {
            let [t, n] = (e.currentTarget as HTMLInputElement).value.split(":");
            t &&
            n &&
            ((t = Math.min(parseInt(t, 10), 23).toString().padStart(2, "0")),
              (n = Math.min(parseInt(n, 10), 59).toString().padStart(2, "0")),
              ((e.currentTarget as HTMLInputElement).value = `${t}:${n}`));
        });
